...
u16 Get_Adc(u32 ch)   
{
    ...
    ADC1_ChanConf.SamplingTime=ADC_SAMPLETIME_1CYCLE_5;      //采样时间    改为最快           
...
//    }
    ...
    for(i=0; i<NumMeasurePoints; i++){
        temp = (float)OrginalV[i]*(3.3/4096.0);		    //得到ADC电压值(现在是真实值，最后需换为转换真实值)
        temp = (temp - 1.44)/2.32*5.0 * 1000.0;          //跟外置电路有关
        True_mV[i] = temp;
        if(max_mV < True_mV[i]){
            max_mV = temp;
        }
